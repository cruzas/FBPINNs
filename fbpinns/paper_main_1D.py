...
import numpy as np
import problems
from active_schedulers import (
    AllActiveSchedulerND,
    LineActiveSchedulerND,
    PlaneActiveSchedulerND,
    PointActiveSchedulerND,
)
from constants import Constants, get_subdomain_ws, get_subdomain_xs
...
def run_PINN():
    ...
    c = Constants(
        RUN="final_PINN_%s_%sh_%sl_%sb_%s"
        % (P.name, n_hidden, n_layers, batch_size[0], sampler),
        P=P,
...
width = 0.7
subdomain_ws = get_subdomain_ws(subdomain_xs, width)
for A, args, n_steps in [
    (AllActiveSchedulerND, (), 50000),
    (
        PointActiveSchedulerND,
        (
            np.array(
