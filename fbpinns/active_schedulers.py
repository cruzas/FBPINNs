...
class AllActiveSchedulerND(_ActiveScheduler):
    ...
    def __iter__(self):
        ...
        for i in range(self.N_STEPS):
            if i == 0:
                yield np.ones(self.nm, dtype=int)  # (nm)
            else:
                yield None
...
class _SubspacePointActiveSchedulerND(_ActiveScheduler):
    ...
    def __init__(self, N_STEPS, D, point, iaxes):
        ...
        # validation
        if point.ndim != 1:
            raise Exception("ERROR: point ndim !=1")
        if len(point) > self.nd:
            raise Exception("ERROR: len point > self.nd")
        if len(iaxes) + len(point) != self.nd:
            raise Exception("ERROR: len iaxes + len point != nd")
    ...
    def _get_radii(self, point, xx):
        ...
        c_inside = np.prod(c_inside, axis=0).astype(
            bool
        )  # (nm)    must be true across all dims

        # get bounding corners of each model
        x = np.stack([xmins, xmaxs], axis=0)  # (2, nd, nm)
        bb = np.zeros((2**nd, nd) + nm)  # (2**nd, nd, nm)
        for ic, offsets in enumerate(
            itertools.product(*([[0, 1]] * nd))
        ):  # for each corner
    ...
    def __iter__(self):
        ...
        rmin, rmax = np.expand_dims(rmin, axis=self.iaxes), np.expand_dims(
            rmax, axis=self.iaxes
        )  # (nm with 1s)
        ...
        for i in range(self.N_STEPS):
            ...
            c_radius = (rt >= rmin) & (
                rt < rmax
            )  # (nm) circle inside box (approximately! (only uses corners))
