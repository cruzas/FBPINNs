...
class ActiveRectangularDomainND(domainsBase._RectangularDomainND):
    ...
    def _set_outside_filters(
        ...
        for ioa, ms in enumerate(self.models_segments):  # (ne,nd,nm) ms
            ...
            c = np.prod(c, axis=1).astype(
                bool
            )  # (ne,nm) collapse along ndim (i.e. has to be true across all nd)
            cs.append(c)
        self.cs = cs
    ...
    def _get_window_functions(self, neighbours):
        ...
        for im, ii in itergrid(self.nm):
            ...
            d = neighbours[
                (slice(None),) + sl
            ]  # (nd,2) neighbour values at grid location
            xmin, xmax = (
                self.xmins[sl],
                self.xmaxs[sl],
            ...
            w.append(
                windows.construct_window_function_ND(
    ...
    def _get_isegs(self, ioa, ii):
        ...
        if len(
            iis
        ):  # nc can equal 0 if ioa > 0 and only one model spans a full subdomain axis
    ...
    def update_active(self, active=None):
        ...
        if active.shape != self.nm:
            raise Exception(
                "ERROR: active shape %s does not equal model grid %s!"
                % (active.shape, self.nm)
            )
            ...
            if c_active[ii]:
                ...
                for ioa in range(self.N_ORDERS):
                    ...
                    for iseg, ii_ in zip(isegs, iis):
                        ...
                        i3 = i2 + np.prod(
                            self.batch_ns[ioa][(slice(None),) + tuple(ii_)]
                        )
    ...
    def _set_torch_tensors(self, device):
        ...
        totorch = lambda x: torch.from_numpy(x.copy().astype(np.float32)).to(
            self.device
        )
    ...
    def update_sampler(self, batch_size, random):
        ...
        if len(batch_size) != self.nd:
            raise Exception("ERROR: len(batch_size) != self.nd! (%s)" % (batch_size,))

        # get global properties
        b = np.array(batch_size)  # (nd)
        lim = np.array(
            [[x.min(), x.max()] for x in self.subdomain_xs]
        ).T  # (2, nd) # global limits along each dim
        ...
        for ioa in range(self.N_ORDERS):
            ...
            if random:
                n = np.round(
                    ...
                    int
                )  # (nd,nm) n number of points along each dim
            else:
            ...
            if self.s[iseg]:  # if segment is in the active/fixed neighbour domain
                ...
                if self.random:
                    ...
                    xs_ = [
                        ...
                        + (
                            i1i
                            + torch.arange(ni, dtype=torch.float32, device=self.device)
                        )
                        * di
                        for li, i1i, ni, di in zip(
                            self.batch_lim[0], i1, n, self.batch_d
                        )
                    ]
    ...
    for subdomain_xs, subdomain_ws in [
        ...
        if plot:
            ...
            for im, ii in itergrid(D.nm):
                ...
                if isegs:
                    iiis = np.stack(
                        np.unravel_index(isegs, D.onm), -1
                    )  # grid index of segments
                    ioas = iiis[:, 0]  # order numbers

                    iis = ii + 0.1 * (
                        iiis[:, 1:] - ii
                    )  # shift segment grid points to be around model grid point
                    for i, e in enumerate(iis):
                        plt.scatter(
                            e[0],
                            0,
                            c=plot_domain.colors[ioas[i]],
                            s=10 * (1 + 50 * (D.N_ORDERS - ioas[i])),
                        )
